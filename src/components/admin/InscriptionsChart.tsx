...
const HAUTEUR = 100;
const MARGE = 20;
const REPERES = [0, 0.5, 1];

// Graphique en courbe (SVG, sans dépendance externe) des inscriptions
// jour par jour : hauteur fixe, lignes de repère discrètes, aire remplie
// sous la courbe et repères de dates.
export default function InscriptionsChart({ donnees }: InscriptionsChartProps) {
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-5">
      <span className="mb-3.5 block text-[10.5px] font-bold tracking-wide text-ink-muted uppercase">
        Inscriptions — 30 derniers jours
      </span>
      <div className="h-[150px]">
        <svg viewBox={`0 0 ${LARGEUR} ${HAUTEUR}`} preserveAspectRatio="none" className="h-full w-full">
          {REPERES.map((repere) => (
            <line
              key={repere}
              x1={MARGE}
              y1={MARGE + hauteurUtile * repere}
              x2={MARGE + largeurUtile}
              y2={MARGE + hauteurUtile * repere}
              stroke="var(--color-line)"
              strokeWidth={1}
            />
          ))}
          <polygon points={aire} fill="var(--color-accent)" fillOpacity={0.12} />
          <polyline points={ligne} fill="none" stroke="var(--color-accent)" strokeWidth={2.5} />
          {points.map(
            (p) =>
              p.jour.count > 0 && (
